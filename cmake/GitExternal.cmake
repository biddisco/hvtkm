...
function(GIT_EXTERNAL DIR REPO TAG)
  ...
  if (GIT_EXTERNAL_LOCAL_OPTION_VERBOSE)
    message("Setting VERBOSE to true")
    set(GIT_EXTERNAL_VERBOSE 1)
  endif()
  ...
  if(NOT EXISTS "${DIR}")
    ...
    if (GIT_EXTERNAL_LOCAL_OPTION_SHALLOW)
      set(_command clone --recursive --depth 5 --branch ${TAG} ${REPO} ${DIR})
    else()
      set(_command clone --recursive ${REPO} ${DIR})
    endif()
    git_external_message("Executing git " ${_command})
    execute_process(
    ...
    # after the intial clone, we can exit as we are already on the required branch
  ...
  if(NOT FRESH_CLONE AND (GIT_EXTERNAL_DISABLE_UPDATE OR GIT_EXTERNAL_LOCAL_OPTION_DISABLE_UPDATE))
